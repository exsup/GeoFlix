...
const MenuGridItem = ({ singleProduct, handleAddProduct, handleRemoveProduct }) => {
  ...
  return (
    <article className="menu-grid-item flex-container flex-column txt-white">
      <Link onClick={ResetLocation} to={`/menu/Ksh{singleProduct.id}`} className="menu-item-link">
        <img src={singleProduct.ItemImg} alt={`Ksh{singleProduct.ItemName}`} />
      </Link>
      ...
      <div className="price">
        {singleProduct.sale === true ?
          <section className="sale-pricing">
            <p className="price-num-before"><span>Ksh</span>{singleProduct.ItemPriceBefore}</p>
            <p className="price-num"><span>Ksh</span>{singleProduct.ItemPrice}</p>
          </section>
          :
          <p className="price-num"><span>Ksh</span>{singleProduct.ItemPrice}</p>
        }
